...
N_sim = 1000;
dist_SR_range = (0.2:0.2:1.4);

power_my_2 = zeros(1,length(dist_SR_range));
power_my_3 = zeros(1,length(dist_SR_range));
power_my_4 = zeros(1,length(dist_SR_range));
power_my_5 = zeros(1,length(dist_SR_range));
power_my = zeros(1,length(dist_SR_range));
power_wo_SR = zeros(1,length(dist_SR_range));
power_BS_SR = zeros(1,length(dist_SR_range));
...
for sim = 1:N_sim
    ...
    power_wo_SR = power_wo_SR + 10*log10(1000*min(p6)/4^K).*ones(1,length(dist_SR_range));

    for dist_index = 1:length(dist_SR_range)
        dist_SR = dist_SR_range(dist_index)
...
figure
plot(dist_SR_range,power_my,'-o','color',[0.8,0,0],'LineWidth',1.5)
hold on
plot(dist_SR_range,power_my_2,'-s','color',[0,0.4,0.8],'LineWidth',1.5)
plot(dist_SR_range,power_my_3,'-d','color',[0,0.7,0.7],'LineWidth',1.5)
plot(dist_SR_range,power_my_4,'-+','color',[0.15,0.15,0.15],'LineWidth',1.5)
plot(dist_SR_range,power_my_5,'-x','color',[0,0.5,0],'LineWidth',1.5)
plot(dist_SR_range,power_wo_SR,'-^','color',[1,0.3,0.6],'LineWidth',1.5)
plot(dist_SR_range,power_BS_SR-10*log10(0.5),'-*','color',[0.5,0.5,0],'LineWidth',1.5)
hold off
xlabel('SIRs QoS requirement \beta');
ylabel('Average transmit power (dBm)');
grid on
% axis([2 3.2 18.5 25.5])
set(gca,'XTicklabel',{'0.2\sigma','0.4\sigma','0.6\sigma','0.8\sigma','\sigma','1.2\sigma','1.4\sigma',})
